class AncientTradeGame {
  ...
  updateUI() {
    ...
    if (phaseInfo) {
      phaseInfo.innerHTML = `
        <div>回合: ${this.round}, 阶段: <span class="highlight-phase">${this.getPhaseDisplayName()}</span></div>
        <div>当前行动: ${this.getCurrentPlayer().name}</div>
        <div>行动点: <span class="highlight-ap">${this.getCurrentPlayer().actionPoints}</span></div>
      `;
    }
  ...
  // 增强 updateActionButtons 方法，添加视觉反馈
  updateActionButtons() {
    ...
    switch(this.currentPhase) {
      case 'planning':
        this.disableButton(drawCardBtn, 'disabled-wrong-phase', '准备阶段不能抽卡');
        this.disableButton(buildRouteBtn, 'disabled-wrong-phase', '准备阶段不能建立路线');
        this.disableButton(tradeBtn, 'disabled-wrong-phase', '准备阶段不能交易');
        break;
      case 'movement':
        if (noActionPoints) {
          this.disableButton(drawCardBtn, 'disabled-no-ap', '没有足够的行动点');
        } else {
          drawCardBtn.disabled = false;
          drawCardBtn.title = '使用1行动点抽取一张卡牌';
        }
        this.disableButton(buildRouteBtn, 'disabled-wrong-phase', '移动阶段不能建立路线');
        this.disableButton(tradeBtn, 'disabled-wrong-phase', '移动阶段不能交易');
        break;
      case 'trading':
        if (noActionPoints) {
          this.disableButton(drawCardBtn, 'disabled-no-ap', '没有足够的行动点');
        ...
        this.disableButton(buildRouteBtn, 'disabled-wrong-phase', '交易阶段不能建立路线');
        tradeBtn.disabled = false;
        tradeBtn.title = '与其他玩家交换资源';
        break;
      case 'building':
        this.disableButton(drawCardBtn, 'disabled-wrong-phase', '建设阶段不能抽卡');
        buildRouteBtn.disabled = false;
        buildRouteBtn.title = '使用资源建立贸易路线';
        this.disableButton(tradeBtn, 'disabled-wrong-phase', '建设阶段不能交易');
        break;
      case 'ending':
        this.disableButton(drawCardBtn, 'disabled-wrong-phase', '结束阶段不能抽卡');
        this.disableButton(buildRouteBtn, 'disabled-wrong-phase', '结束阶段不能建立路线');
        this.disableButton(tradeBtn, 'disabled-wrong-phase', '结束阶段不能交易');
        break;
    }
  }
  ...
  updatePlayerInfoPanel() {
    ...
    panel.innerHTML = `
      <h3>玩家: ${player.name}</h3>
      <p>颜色: <span style="color:${player.color}">${player.color}</span></p>
      <p>行动点: ${player.actionPoints}</p>
      <p>胜利点: ${player.victoryPoints}</p>
      <p>文化点: ${player.culturalPoints}</p>
      <h4>手牌:</h4>
    ...
    player.secretTasks.forEach(task => {
      ...
      if (task.completed) {
        taskElement.textContent += ` (已完成: +${task.reward}分)`;
      }
