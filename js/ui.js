  ...
  document.getElementById('build-route-btn').addEventListener('click', function() {
    ...
    if (game.currentPhase === 'building') {
      ...
      const cityOptions = Object.entries(game.gameBoard.cities)
        .map(([id, city]) => {
          const cityName = game.gameBoard.nameTranslations.cities[id][lang];
      ...
      dialog.innerHTML = `
        <h3>${lang === 'en' ? 'Establish Trade Route' : '建立贸易路线'}</h3>
        <div>
          <label for="from-city">${lang === 'en' ? 'Starting City:' : '起点城市:'}</label>
          <select id="from-city">${cityOptions}</select>
        </div>
        <div style="margin-top:10px;">
          <label for="to-city">${lang === 'en' ? 'Destination City:' : '终点城市:'}</label>
          <select id="to-city">${cityOptions}</select>
        </div>
        <p>${lang === 'en' ? 'Select resource cards for route:' : '选择资源卡用于铺设路线:'}</p>
        <div id="resource-selection-cards"></div>
        <div class="dialog-buttons">
          <button id="confirm-route">${window.translate('confirm')}</button>
          <button id="cancel-route">${window.translate('cancel')}</button>
        </div>
      `;
...
// 修改showCultureInvestmentDialog函数
function showCultureInvestmentDialog(cityId) {
  ...
  if (culturalCards.length === 0) {
    alert(lang === 'en' ? 
      'You have no cards for cultural projects (academic resources or textiles)' : 
      '你没有可用于文化工程的卡牌（学术资源或纺织品）');
    return;
  }
  
  // 获取城市名称的翻译
  const cityName = game.gameBoard.nameTranslations.cities[cityId][lang];
  
  // 创建对话框
  const dialog = document.createElement('div');
  dialog.className = 'dialog';
  dialog.innerHTML = `
    <h3>${lang === 'en' ? `Invest in Cultural Project in ${cityName}` : `在${cityName}投资文化工程`}</h3>
    <p>${lang === 'en' ? 'Select cards to invest:' : '选择用于投资的卡牌:'}</p>
    <div id="cultural-cards"></div>
    <div class="dialog-buttons">
      <button id="confirm-culture">${window.translate('confirm')}</button>
      <button id="cancel-culture">${window.translate('cancel')}</button>
    </div>
  `;
